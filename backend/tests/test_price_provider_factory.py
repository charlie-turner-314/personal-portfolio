...
def test_factory_returns_yahoo_provider_with_no_env_vars():
    with patch.dict(os.environ, {}, clear=False):
        for var in ("PERSONAL_PORTFOLIO_PRICE_PROVIDER", "PERSONAL_PORTFOLIO_PRICE_PROVIDER_API_KEY"):
...
def test_factory_ignores_legacy_env_vars():
    ...
    with patch.dict(
        os.environ,
        {
            "PERSONAL_PORTFOLIO_PRICE_PROVIDER": "alpha_vantage",
            "PERSONAL_PORTFOLIO_PRICE_PROVIDER_API_KEY": "irrelevant",
        },
    ):
        provider = get_price_provider()
    assert isinstance(provider, YahooPriceProvider)
